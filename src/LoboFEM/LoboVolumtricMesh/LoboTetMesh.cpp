    ...
    if (lobomesh_binding != NULL)
    {
        ...
        for (int i = 0; i < numtrinode; i++)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                ...
                for (int k = 0; k < 3; k++)
                {
                    buffer.data()[i * 3 + k] += tet_vertice.data()[nodeid * 3 + k] * tri_ele_weights.data()[i * 4 + j];
                }
            }
        }
...
void Lobo::LoboTetMesh::getNodeElements(std::vector<std::vector<int>> &node_elements)
{
...
void Lobo::LoboTetMesh::computeElementShapeFunctionDerivate(int elementid)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        te->Phi_derivate.data()[0 * 4 + i] = inverseShapefunction.data()[0 * 4 + i];
        te->Phi_derivate.data()[1 * 4 + i] = inverseShapefunction.data()[1 * 4 + i];
        te->Phi_derivate.data()[2 * 4 + i] = inverseShapefunction.data()[2 * 4 + i];
    }
}
