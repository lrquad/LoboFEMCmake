...
void Lobo::LoboMesh::drawImGui(bool *p_open)
{
    ...
    if (ImGui::CollapsingHeader("Info", ImGuiWindowFlags_NoCollapse))
    {
        ...
        {
            ImGui::Text("num shapes faces: %d ", shapes[s].mesh.num_face_vertices.size());
        }
        ...
        if (ImGui::CollapsingHeader("Materials"))
        {
            ...
            {
                ImGui::Text("material[%d].diffuse_texname: %s", int(i), materials[i].diffuse_texname.c_str());
                ImGui::Text("material[%d].diffuse_color: %f %f %f", int(i),
                            materials[i].diffuse[0], materials[i].diffuse[1], materials[i].diffuse[2]);
                ImGui::Text("material id: %d", material_buffer[i].diffuse_texid);
            }
        }
    }
...
void Lobo::LoboMesh::initialGL()
{
    ...
    for (int i = 0; i < num_shapes; i++)
    {
        ...
        int buffersize = shapes[i].mesh.indices.size() * (shape_buffer[i].size_per_vertex);
        shape_buffer[i].vb.resize(buffersize); //pos(3 float) normal(3 float), tex(2 float), color(3 float)
        //convert buffer
        ...
        glBufferData(GL_ARRAY_BUFFER, shape_buffer[i].vb.size() * sizeof(float), &(shape_buffer[i].vb)[0], GL_STATIC_DRAW);
    ...
    for (size_t m = 0; m < materials.size(); m++)
    {
        ...
        if (mp->diffuse_texname.length() > 0)
        {
            ...
            bindTextureBuffer(mp->diffuse_texname.c_str(), material_buffer[m].diffuse_texid);
...
void Lobo::LoboMesh::updateShapeArrayBuffer(int shape_id)
{
    ...
    for (int j = 0; j < shapes[shape_id].mesh.indices.size(); j++)
    {
        ...
        shape_buffer[shape_id].vb[j * size_per_vertex + 0] = attrib.vertices[vid * 3 + 0];
        shape_buffer[shape_id].vb[j * size_per_vertex + 1] = attrib.vertices[vid * 3 + 1];
        shape_buffer[shape_id].vb[j * size_per_vertex + 2] = attrib.vertices[vid * 3 + 2];

        shape_buffer[shape_id].vb[j * size_per_vertex + 8] = vertex_color[vid * 3 + 0];
        shape_buffer[shape_id].vb[j * size_per_vertex + 9] = vertex_color[vid * 3 + 1];
        shape_buffer[shape_id].vb[j * size_per_vertex + 10] = vertex_color[vid * 3 + 2];

        int nid = shapes[shape_id].mesh.indices[j].normal_index;

        if (nid != -1)
        {
            shape_buffer[shape_id].vb[j * size_per_vertex + 3] = attrib.normals[nid * 3 + 0];
            shape_buffer[shape_id].vb[j * size_per_vertex + 4] = attrib.normals[nid * 3 + 1];
            shape_buffer[shape_id].vb[j * size_per_vertex + 5] = attrib.normals[nid * 3 + 2];
        }
        ...
        {
            shape_buffer[shape_id].vb[j * size_per_vertex + 6] = attrib.texcoords[tid * 2 + 0];
            shape_buffer[shape_id].vb[j * size_per_vertex + 7] = attrib.texcoords[tid * 2 + 1];
        }
    }
}
...
void Lobo::LoboMesh::paintGL(LoboShader *shader)
{
    ...
    for (int i = 0; i < shape_buffer.size(); i++)
    {
        ...
        {
            glm::vec3 color = glm::vec3(materials[materia_id].diffuse[0], materials[materia_id].diffuse[1], materials[materia_id].diffuse[2]);
        ...
        setPositionAttribute(2, 2, 11, 6);
        setPositionAttribute(3, 3, 11, 8);
        glDrawArrays(GL_TRIANGLES, 0, shape_buffer[i].vb.size() / shape_buffer[i].size_per_vertex);
    }
